...
const redeemPromotion = async () => {
  try {
    const apiUrl = new URL(`/api/users/redeem/${userId.value}/${promotion.value.points}`, process.env.VUE_APP_API_URL);
  ...
  <BaseBody>
    ...
    <div class="p-4">
      ...
      <div v-else-if="promotion.title">
        <img :src="promotion.image" alt="Promoción" class="w-full h-aut object-cover rounded mb-4">
