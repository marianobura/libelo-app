...
const daysInMonth = computed(() => {
  return (
    Number(new Date(currentYear.value, currentMonth.value + 1, 0).getDate()) ||
    0
  );
});
...
const firstDayOfMonth = computed(() => {
  return (
    Number(new Date(currentYear.value, currentMonth.value, 1).getDay()) || 0
  );
});

const isCreating = computed(
  () => showEventModal.value && !selectedEvent.value?.id,
);
...
async function saveChanges() {
  const updatedEvent = {
    ...
    start: { dateTime: new Date(formattedEditDate.value).toISOString() },
      ...
      dateTime: new Date(
        new Date(formattedEditDate.value).getTime() + 60 * 60 * 1000,
      ).toISOString(),
    },
  };
...
async function getCalendarEvents() {
  try {
    ...
    const url = `https://www.googleapis.com/calendar/v3/calendars/primary/events?maxResults=100&orderBy=startTime&singleEvents=true&timeMin=${encodeURIComponent(
      start,
  ...
  <BaseBody>
    ...
    <div class="flex flex-col gap-4 p-2">
      ...
      >
        ...
        <div
          class="grid grid-cols-7 text-center font-semibold border-b border-gray-300 pb-2 mb-2"
        >
          <div>Dom</div>
          <div>Lun</div>
          <div>Mar</div>
        ...
        <div
          class="grid grid-cols-7 gap-2 text-center justify-items-center border-b border-gray-300 pb-2"
        >
          ...
          <div
            ...
            :class="[
              selectedDays.includes(day)
                ...
                : isToday(day)
                ? 'text-libelo-500 bg-libelo-100 font-bold'
                : 'hover:bg-libelo-100',
            ]"
          >
            {{ day
            }}<span
              v-if="hasEventOnDay(day) && !selectedDays.includes(day)"
              class="absolute bottom-0 w-4 h-1 rounded-full bg-libelo-500 focus:text-white"
            ></span>
          </div>
        </div>
        <BaseTitle
          v-if="getEventsforSelectedDays().length"
          :title="`Eventos para el día ${selectedDateLabel}`"
        >
          <template v-if="calendarEvents.length">
            <div
              v-for="event in getEventsforSelectedDays()"
              :key="event.id"
              @click="openEventModal(event)"
              class="p-2 flex justify-between items-center text-white bg-libelo-500 hover:bg-libelo-600 rounded-xl overflow-hidden"
            >
              ...
              <button
                class="size-11 flex items-center justify-center hover:bg-libelo-600 rounded-lg shrink-0"
