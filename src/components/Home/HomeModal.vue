...
<template>
    <BaseModal v-if="props.showModal" class="items-end justify-center" @click="handleOverlayClick">
        <div class="grid grid-rows-modal bg-white p-4 rounded-t-xl w-full h-modal">
            ...
            <div class="pb-4 overflow-scroll">
                <ul class="flex flex-col gap-4">
                    <li v-for="(subjects, category) in filteredSubjects" :key="category">
                        ...
                        <ul class="pl-2">
                            <li v-for="subject in subjects" :key="subject" class="p-2 cursor-pointer hover:bg-neutral-100" @click="subjectSelected = subject">{{ subject }}</li>
                        </ul>
                    </li>
                </ul>
            </div>
